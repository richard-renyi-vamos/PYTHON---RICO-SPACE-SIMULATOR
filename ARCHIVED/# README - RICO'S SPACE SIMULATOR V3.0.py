...
PLANS: 
► split up the main PY code
► use several modules for different functions

---------------

Add Music Toggle to Game GUI
1

import pygame import sys import random import tkinter as tk from tkinter import ttk from SPACE_OBJECTS import SpaceObject # Import the new SpaceObject class # Initialize Pygame pygame.init()
PASTED
R
Modify this PYTHON game GUI to have a music option.
► The music should be ON in default, but should be able to turn it OFF.
► The music is "Starry Dream.mp3" in the same folder

Certainly! I'll modify the game GUI to include a music option that allows the user to turn the music on and off. The music will be on by default. Here's the modified version of the code:
...
root.destroy()
sys.exit()
Here are the changes I made to add the music option:

Initialized the Pygame mixer: pygame.mixer.init()
Loaded and started playing the music:
python
...
pygame.mixer.music.play(-1)  # -1 means loop indefinitely
music_playing = True
Added a music toggle button to the SettingsGUI class:
python

Copy
self.music_button = ttk.Button(master, text="Music: ON", command=self.toggle_music)
self.music_button.grid(row=4, column=0, columnspan=3, pady=10)
Implemented the toggle_music method in the SettingsGUI class:
python
...
def toggle_music(self):
    ...
    if music_playing:
        ...
        self.music_button.config(text="Music: ON")
        music_playing = True
These modifications add a music toggle button to the settings GUI. The music starts playing by default when the game is launched. Clicking the "Music: ON" button will pause the music and change the button text to "Music: OFF". Clicking it again will resume the music and change the text back to "Music: ON".

The music file "Starry Dream.mp3" should be in the same folder as the Python script for this to work correctly.
